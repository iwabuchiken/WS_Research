...
def create_PPM_File(fname)
  
  #########################
  # open: file
  #########################
  #REF http://ruby.bastardsbook.com/chapters/io/
  f = File.open(fname, "w")
  ...
  puts "[#{File.basename(__FILE__)} : #{__LINE__}] ppm file => created: #{fname}"
    
end

def get_Time_String_Now
  
  #REF http://alvinalexander.com/blog/post/ruby/how-determine-current-date-time-ruby
  now = Time.now  #=> 2015-07-07 10:56:30 +0900
  ...
  t_Date = tokens[0].split("-")
  
  label_Date = t_Date.join("")

  #########################
  # time label
  #########################
  t_Time = tokens[1].split(":")
    
  label_Time = t_Time.join("")
  
  return "#{label_Date}_#{label_Time}"
