...
#REF http://stackoverflow.com/questions/3594388/ruby-require-path answered Aug 29 '10 at 17:08
...
def exec
  ...
  # s-5: ppm file
  #########################
  fname = "#{File.dirname(File.dirname(__FILE__))}/data/s-5/test.ppm"
#  C:\WORKS\WS\WS_Research\R.150-199\R.165_image-proccessing\data\s-5
  
  puts "[#{File.basename(__FILE__)} : #{__LINE__}] fname => #{fname}"
  
  
  puts "[#{File.basename(__FILE__)} : #{__LINE__}] now => #{Time.now}"
  
  
#  fname = "#{File.dirname(File.dirname(__FILE__))}/data/s-t/test.ppm"
  
  create_PPM_File(fname)
